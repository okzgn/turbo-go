...
// RequestFromMap creates an Request from CGI variables.
...
// DefaultServeMux.
func CgiServe(handler Handler) error {
	req, err := CgiRequest()
	if err != nil {
		return err
	}
...
func (r *cgiresponse) Flush() {
	r.bufw.Flush()
}

func (r *cgiresponse) Header() Header {
	return r.header
}

func (r *cgiresponse) Write(p []byte) (n int, err error) {
	if !r.wroteHeader {
...
func (r *cgiresponse) WriteHeader(code int) {
	if r.wroteHeader {
...
// set explicitly.
func (r *cgiresponse) writeCGIHeader(p []byte) {
	if r.wroteCGIHeader {
		return
	}
	...
	if _, hasType := r.header["Content-Type"]; !hasType {
		r.header.Set("Content-Type", DetectContentType(p))
	}
