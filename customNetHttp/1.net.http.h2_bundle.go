...
func (mh *http2MetaHeadersFrame) RegularFields() []HeaderField {
...
// The caller does not own the returned slice.
func (mh *http2MetaHeadersFrame) PseudoFields() []HeaderField {
	for i, hf := range mh.Fields {
		if !hf.IsPseudo() {
			return mh.Fields[:i]
		}
...
func (fr *http2Framer) readMetaFrame(hf *http2HeadersFrame) (*http2MetaHeadersFrame, error) {
	...
	var sawRegular bool
	var headersLarge uint32

	var invalid error // pseudo header field errors
	hdec := fr.ReadMetaHeaders
	hdec.SetEmitEnabled(true)
	// Okzgn: changes on hpack for Y instead of d.maxStrLen
	//hdec.SetMaxStringLength(fr.maxHeaderStringLen())
	hdec.SetEmitFunc(func(hf HeaderField) {
		...
		size := hf.Size()
		headersLarge += size - 32

		if headersLarge > uint32(YH) {
			hdec.SetEmitEnabled(false)
			mh.Truncated = true
			return
		}
	...
	defer hdec.SetEmitFunc(func(hf HeaderField) {})
...
// name (key). See ValidHeaderName for the base rules.
//
// Further, http2 says:
//
...
func (s *http2Server) ServeConn(c net.Conn, opts *http2ServeConnOpts) {
	...
	sc.hpackEncoder = NewEncoder(&sc.headerWriteBuf)
	...
	fr.ReadMetaHeaders = NewDecoder(s.maxDecoderHeaderTableSize(), nil)
...
func (sc *http2serverConn) HeaderEncoder() (*Encoder, *bytes.Buffer) {
...
func (sc *http2serverConn) serve() {
	...
	if V["IT"] != 0 {
		sc.idleTimer = time.AfterFunc(V["IT"], sc.onIdleTimer)
...
func (sc *http2serverConn) processHeaders(f *http2MetaHeadersFrame) error {
	...
	if f.URITooLarge {
		// URI was too long. Send a 414 error.
		handler = http2handleURIOrHeaderTooLong(414)
	}
	if f.Truncated {
		// Their header list was too long. Send a 431 error.
		handler = http2handleURIOrHeaderTooLong(431)
	...
	if V["RT"] != 0 {
		...
		if st.body != nil {
			st.readDeadline = time.AfterFunc(V["RT"], st.onReadTimeout)
		}
	}
...
func (sc *http2serverConn) newStream(id, pusherID uint32, state http2streamState) *http2stream {
	...
	if V["WT"] != 0 {
		st.writeDeadline = time.AfterFunc(V["WT"], st.onWriteTimeout)
	}
...
/*func http2handleHeaderListTooLong(w ResponseWriter, r *Request, c int) {
...
func http2handleURIOrHeaderTooLong(c int) HandlerFunc {
...
func (t *http2Transport) pingTimeout() time.Duration {
	if t.PingTimeout == 0 {
		return 5 * time.Second // Default: 15s
	}
...
func (t *http2Transport) newClientConn(c net.Conn, singleUse bool) (*http2ClientConn, error) {
	...
	if d := V["IT"]; d != 0 {
		cc.idleTimer = time.AfterFunc(V["IT"], cc.onIdleTimeout)
	}
	if http2VerboseLogs {
	...
	cc.fr.ReadMetaHeaders = NewDecoder(maxHeaderTableSize, nil)
	cc.fr.MaxHeaderListSize = t.maxHeaderListSize()

	cc.henc = NewEncoder(&cc.hbuf)
...
func (cc *http2ClientConn) tooIdleLocked() bool {
	...
	return V["IT"] != 0 && !cc.lastIdle.IsZero() && time.Since(cc.lastIdle.Round(0)) > V["IT"]
}
...
func (cs *http2clientStream) writeRequest(req *Request) (err error) {
	...
	if continueTimeout != 0 {
		if !HeaderValuesContainsToken(req.Header["Expect"], "100-continue") {
			continueTimeout = 0
		} else {
...
func (cc *http2ClientConn) writeHeader(name, value string) {
	...
	cc.henc.WriteField(HeaderField{Name: name, Value: value})
}
...
func http2isConnectionCloseRequest(req *Request) bool {
	return req.Close || HeaderValuesContainsToken(req.Header["Connection"], "close")
}
...
type http2writeContext interface {
	...
	// returned buffer.
	HeaderEncoder() (*Encoder, *bytes.Buffer)
}
...
func http2encKV(enc *Encoder, k, v string) {
	if http2VerboseLogs {
		log.Printf("http2: server encoding header %q = %q", k, v)
	}
	enc.WriteField(HeaderField{Name: k, Value: v})
}
...
func http2encodeHeaders(enc *Encoder, h Header, keys []string) {
	if keys == nil {
		sorter := http2sorterPool.Get().(*http2sorter)
