...
func (r *Request) MultipartReader() (*Reader, error) {
...
func (r *Request) multipartReader(allowMixed bool) (*Reader, error) {
...
func readRequest(b *bufio.Reader) (req *Request, err error) {
	...
	if len(req.RequestURI) > YU {
		return nil, errURITooLarge /*badStringError("URI length limit exceeded", "")*/
	}
...
func parsePostForm(r *Request) (vs url.Values, err error) {
	...
	case ct == "application/x-www-form-urlencoded":
		...
		maxFormSize := L_ // okzgn was: int64(1<<63 - 1)

		if _, ok := r.Body.(*maxBytesReader); !ok {
			maxFormSize = L_ // okzgn was: int64(10 << 20). 10 MB is a lot of text.
...
func (r *Request) FormFile(key string) (MimeFile, *FileHeader, error) {
	if r.MultipartForm == multipartByReader {
		return nil, nil, errors.New("http: multipart handled by MultipartReader")
	}
	if r.MultipartForm == nil {
		err := r.ParseMultipartForm(L_)
		if err != nil {
			return nil, nil, err
		}
	}
