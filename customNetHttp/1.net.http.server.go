...
//const DefaultMaxHeaderBytes = 1000 // 1 << 20 = 1 MB
...
var errTooLarge = errors.New("http: request headers too large")
var errURITooLarge = errors.New("http: request URI too large")
...
// well read them)
const maxPostHandlerReadBytes = 0 //okzgn was: 256 << 10
...
func (cw *chunkWriter) writeHeader(p []byte) {
	...
	for k := range cw.header {
		//okzgn: here is the list of particular headers
	...
	if cw.chunking {
		delHeader("Content-Length")
		if tax != 0 {
			//fmt.Println("okzgn 12 err")
		}
	}
	if !w.req.ProtoAtLeast(1, 0) {
		if tax != 0 {
			//fmt.Println("okzgn 13 err")
		}
		return
	}
...
func (c *conn) serve(ctx context.Context) {
	...
	defer func() {
		if err := recover(); err != nil && err != ErrAbortHandler {
			...
			/*c.close()                                // okzgn add
			c.setState(c.rwc, StateClosed, runHooks) // okzgn add*/
			// okzgn: if not is closed here, an error occurs when a idle connection is stoped and the client try to request something
		}
	...
	for {
		...
		if err != nil {
			...
			case isUnsupportedTEError(err):
				...
				ErrorD(c.rwc, "Unsupported transfer encoding", 501)
				return
...
func Error(w ResponseWriter, error string, code int) {
	...
	w.WriteHeader(code)
	w.Header().Set("Content-Type", "text/"+m+"; charset=UTF-8")
	w.Header().Set("X-Content-Type-Options", "nosniff")
	fmt.Fprintln(w, strings.ReplaceAll(v, "{TURBO_RESPONSE_CODE}", strconv.Itoa(code))+"\n"+error)
}
func ErrorD(w net.Conn, error string, code int) {
	v, o := Y[code]
	if !o {
		v = Y[0]
	}
	...
	fmt.Fprintf(w, "HTTP/1.1 "+strconv.Itoa(code)+" "+StatusText(code)+"\r\nContent-Type: text/"+m+"; charset=UTF-8\r\nX-Content-Type-Options: nosniff\r\nServer: "+S_["T"]+"\r\nConnection: close\r\n\r\n")
	fmt.Fprintln(w, strings.ReplaceAll(v, "{TURBO_RESPONSE_CODE}", strconv.Itoa(code))+"\n"+error)
}

// NotFound replies to the request with an HTTP 404 not found error.
func NotFound(w ResponseWriter, r *Request) { Error(w, "", StatusNotFound) }
...
var http2server = InternalGodebugNew("http2server")
